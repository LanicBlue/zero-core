...
const FILE_DESC: Record<string, string> = {
	...
	// Wiki v2（plan-01+，独立 wiki.db 单一实现；详见 src/server/wiki/）
	"src/server/wiki/wiki-database.ts": "WikiDatabase：wiki.db 单一生命周期所有者（open/close/health/checkpoint，WAL+FK+busy_timeout，固定根 bootstrap），DatabaseManager 持有为单实例",
	"src/server/wiki/wiki-schema.ts": "Wiki v2 schema：7 张表 + FTS5 + 触发器 + 索引 + 固定根 SQL 定义",
	"src/server/wiki/wiki-path.ts": "canonical path 工具：节点主键是规范路径字符串（无 internal id 暴露）",
	"src/server/wiki/wiki-node-repository.ts": "WikiNodeRepository：CRUD 节点（canonical path 主键）",
	"src/server/wiki/wiki-link-repository.ts": "WikiLinkRepository：节点间链接关系持久化",
	"src/server/wiki/wiki-audit-repository.ts": "WikiAuditRepository：审计事件流水",
	"src/server/wiki/wiki-repository-store.ts": "仓储层 facade / 事务编排",
	"src/server/wiki/wiki-source-service.ts": "WikiSourceService：项目源码扫描产出 structure（structure-sync）",
	"src/server/wiki/wiki-source-search.ts": "源码侧 full-text / 模糊搜索",
	"src/server/wiki/wiki-address-service.ts": "WikiAddressService：地址解析（canonical path → 节点）",
	"src/server/wiki/wiki-authorization-service.ts": "WikiAuthorizationService：grants 评估，产出 CompiledWikiAccess",
	"src/server/wiki/wiki-access-compiler.ts": "CompiledWikiAccess：grants + context 编译为可见路径集合",
	"src/server/wiki/wiki-context-compiler.ts": "WikiContextCompiler：deep 编译，把 grants/context 物化为 agent 运行时上下文",
	"src/server/wiki/wiki-edit-service.ts": "WikiEditService：写入路径（编辑/创建节点，触发 semantic-sync）",
	"src/server/wiki/wiki-search-service.ts": "WikiSearchService：FTS5 全文搜索（agent 工具入口）",
	"src/server/wiki/wiki-service.ts": "WikiService：data plane facade（read + search）",
	"src/server/wiki/wiki-runtime.ts": "WikiRuntime：组装各 service 的运行时入口",
	"src/server/wiki/wiki-errors.ts": "Wiki 错误类型 / 错误码",
	"src/server/wiki/index.ts": "Wiki 子系统对外导出",
	"src/server/wiki-skeleton-service.ts": "WikiSkeletonService：orchestrator（plan-01+，stub 已在 P1-6 清除）",
	"src/server/wiki-operations.ts": "Wiki 运维操作（management plane 入口）",
	"src/server/wiki-router.ts": "Wiki HTTP/IPC data plane 路由",
	"src/server/wiki-admin-router.ts": "Wiki 管理 plane 路由（grants / 用户管理）",
	"src/server/wiki-maintenance-router.ts": "Wiki 维护 plane 路由（备份 / reindex / vacuum）",
	"src/server/wiki-backup-service.ts": "WikiBackupService：SQLite Backup API 单一所有者（design.md §3.5）",

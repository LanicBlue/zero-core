...
const FUNC_DESC: Record<string, string> = {
	...
	"src/runtime/tools/index.ts/getAssistantTools:29": "懒加载并缓存 Assistant 工具实例",
	"src/runtime/tools/index.ts/registerRuntimeTools:76": "将所有内置工具注册到全局 ToolRegistry",
	"src/runtime/tools/index.ts/buildToolsSet:99": "根据策略构建当前请求可用的工具集",
	"src/runtime/tools/index.ts/getToolCategories:185": "返回按类别分组的工具名映射表",
	"src/runtime/tools/index.ts/getAllToolInfo:198": "返回所有工具的元信息摘要列表",
	"src/runtime/tools/index.ts/buildToolPolicyDescription:216": "生成描述当前工具启用/禁用/读写权限的文本",
	// ── runtime/tools/agent-tool.ts ──
	"src/runtime/tools/agent-tool.ts/kebabCase:18": "将字符串转为 kebab-case 格式",
	"src/runtime/tools/agent-tool.ts/resolveTemplate:25": "替换模板中的 {{task}} 占位符",
	"src/runtime/tools/agent-tool.ts/resolveArgsTemplate:29": "解析模板并按引号规则拆分为参数数组",
	"src/runtime/tools/agent-tool.ts/truncateResult:47": "将过长的结果文本截断并附加截断提示",
	...
	"src/runtime/tools/web-search.ts/SearXNGProvider.search:89": "调用 SearXNG JSON API 执行搜索",
	"src/runtime/tools/web-search.ts/SerpAPIProvider.constructor:120": "初始化 SerpAPI 搜索客户端",
	"src/runtime/tools/web-search.ts/SerpAPIProvider.search:124": "调用 SerpAPI Google 引擎执行搜索",
	"src/runtime/tools/web-search.ts/BraveSearchProvider.constructor:149": "初始化 Brave Search 搜索客户端",
	"src/runtime/tools/web-search.ts/BraveSearchProvider.search:157": "调用 Brave Search API 执行搜索",
	...
	"src/runtime/tools/mcp-tool.ts/createMcpTool:8": "从 MCP 工具定义创建 AI SDK 兼容的工具实例",
	"src/runtime/tools/mcp-tool.ts/inputSchemaToZod:40": "将 MCP JSON Schema 转换为 Zod schema",
	"src/runtime/tools/mcp-tool.ts/propToZod:62": "将单个 JSON Schema 属性映射为对应的 Zod 类型",
	"src/runtime/tools/mcp-tool.ts/buildMcpTools:84": "批量构建 MCP 工具映射表",
	// ── runtime/tools/tool-factory.ts ──
	"src/runtime/tools/tool-factory.ts/truncateResult:43": "超过最大长度时截断结果并附加截断说明",
	"src/runtime/tools/tool-factory.ts/buildTool:63": "包装 AI SDK tool() 添加元数据、钩子和结果截断",
	"src/runtime/tools/tool-factory.ts/getToolMeta:154": "从工具对象读取元数据",
	"src/runtime/tools/tool-factory.ts/getToolName:158": "从工具对象读取工具名称",
	"src/runtime/tools/tool-factory.ts/getToolDescription:162": "从工具对象读取短描述",
	"src/runtime/tools/tool-factory.ts/getToolPrompt:166": "从工具对象读取完整提示词",
	"src/runtime/tools/tool-factory.ts/getToolConfigSchema:170": "从工具对象读取配置 schema",
	// ── runtime/tools/task-list.ts ──
	"src/runtime/tools/task-list.ts/formatTask:5": "将任务信息格式化为可读的单行文本",
	// ── runtime/tools/task-status.ts ──
	"src/runtime/tools/task-status.ts/formatTurn:5": "将单条 turn 格式化为截断后的可读文本",
	"src/runtime/tools/task-status.ts/taskStatusTool.execute:48": "查询任务状态并展示最近的会话活动记录",
	// ── runtime/tools/task-stop.ts ──
	"src/runtime/tools/task-stop.ts/taskStopTool.execute:13": "验证任务存在且运行中后执行停止操作",
	// ── runtime/tools/wait.ts ──
	"src/runtime/tools/wait.ts/waitTool.execute:16": "挂起执行等待后台任务完成或超时",
	// ── runtime/tools/syntax-check.ts ──
	"src/runtime/tools/syntax-check.ts/checkSyntax:33": "检查源代码的括号匹配和未闭合字符串问题",
	"src/runtime/tools/syntax-check.ts/checkUnterminatedStrings:79": "检查 C 风格语言中未闭合的引号字符串",
	"src/runtime/tools/syntax-check.ts/formatDiagnostics:117": "将语法诊断信息格式化为可读的警告文本",
	// ── runtime/tools/file-read-helpers.ts ──
	"src/runtime/tools/file-read-helpers.ts/detectFileType:34": "根据文件扩展名判断文件类型（二进制/图片/PDF/笔记本/文本）",
	"src/runtime/tools/file-read-helpers.ts/decodeBuffer:47": "自动检测编码并解码文件内容为 UTF-8 字符串",
	"src/runtime/tools/file-read-helpers.ts/normalizeLineEndings:66": "将 CRLF 统一为 LF 换行",
	"src/runtime/tools/file-read-helpers.ts/formatImageInfo:74": "生成图片文件的基本信息描述",
	"src/runtime/tools/file-read-helpers.ts/extractPdfText:85": "从 PDF 文件中提取可读文本内容",
	"src/runtime/tools/file-read-helpers.ts/parseJupyterNotebook:111": "解析 Jupyter 笔记本文件并格式化输出代码和结果",
	"src/runtime/tools/file-read-helpers.ts/parsePageRange:160": "解析页码范围字符串为起止索引",
	"src/runtime/tools/file-read-helpers.ts/suggestSimilarFiles:172": "当文件不存在时推荐名称相似的候选文件",
	"src/runtime/tools/file-read-helpers.ts/similarity:192": "计算两个字符串的字符级相似度分数",
	"src/runtime/tools/file-read-helpers.ts/formatBytes:207": "将字节数格式化为人类可读的大小字符串",
	// ── runtime/tools/outline/ ──
	"src/runtime/tools/outline/index.ts/extractOutline:176": "根据文件扩展名选择提取器并生成代码大纲",
	"src/runtime/tools/outline/index.ts/getExtension:191": "从文件路径中提取扩展名，处理 .env 特殊情况",
	"src/runtime/tools/outline/index.ts/fallbackExtract:200": "无专用提取器时按空行分段生成基本大纲",
	"src/runtime/tools/outline/renderer.ts/renderOutline:15": "将大纲树渲染为带行号和缩进的文本，按预算控制展开深度",
	"src/runtime/tools/outline/renderer.ts/countAllNodes:96": "递归统计大纲树中所有节点数",
	"src/runtime/tools/outline/renderer.ts/expandChildren:105": "将子节点展开为渲染条目列表",
	"src/runtime/tools/outline/renderer.ts/expandPriority:115": "根据节点类型返回展开优先级分数",
	"src/runtime/tools/outline/renderer.ts/mergeImports:125": "合并连续的 import 节点以减少输出行数",
	"src/runtime/tools/outline/renderer.ts/fmtLine:159": "格式化单个大纲节点为一行带缩进的文本",
	"src/runtime/tools/outline/renderer.ts/shortKind:171": "将节点类型缩写为短标签",
	"src/runtime/tools/outline/renderer.ts/formatDetail:189": "生成节点的补充详情文本",
	"src/runtime/tools/outline/stripper.ts/stripComments:25": "去除源代码中的注释和字符串内容，保留行号结构",
	// ── runtime/mcp-tools/ ──
	"src/runtime/mcp-tools/assistant-tools.ts/getLatestLogFile:16": "查找日志目录中最新的日志文件",
	"src/runtime/mcp-tools/assistant-tools.ts/redactSensitive:30": "递归遮蔽对象中的 API key、密码等敏感字段",
	"src/runtime/mcp-tools/assistant-tools.ts/createAssistantTools:41": "创建应用诊断工具，支持版本/日志/配置/源码/提供商/文件查询",
	"src/runtime/mcp-tools/fetch-tools.ts/fetchUrl:11": "使用模拟浏览器 UA 获取指定 URL 的响应",
	"src/runtime/mcp-tools/fetch-tools.ts/htmlToText:19": "去除 HTML 中的 script/style 标签并提取纯文本",
	"src/runtime/mcp-tools/memory-tools.ts/getMemoryStore:4": "从数据库上下文中获取记忆存储实例",
	"src/runtime/mcp-tools/sequential-thinking-tools.ts/sequentialThinkingTool.execute:22": "追加一条思考到链中并返回推理进度",
	...
	// ── server/session-db.ts ──
	"src/server/session-db.ts/SessionDB.constructor:20": "初始化会话数据库并执行 schema 和迁移",
	"src/server/session-db.ts/SessionDB.getKVStore:42": "获取键值存储实例",
	"src/server/session-db.ts/SessionDB.getMemoryStore:46": "获取记忆存储实例",
	"src/server/session-db.ts/SessionDB.createSession:107": "创建新会话记录",
	"src/server/session-db.ts/SessionDB.getSession:116": "按 ID 查询会话记录",
	"src/server/session-db.ts/SessionDB.getMainSession:121": "获取指定 agent 的主会话",
	"src/server/session-db.ts/SessionDB.setMainSession:126": "设置指定 agent 的主会话",
	"src/server/session-db.ts/SessionDB.listSessions:134": "列出指定 agent 的所有会话",
	"src/server/session-db.ts/SessionDB.deleteSession:141": "按 ID 删除会话",
	"src/server/session-db.ts/SessionDB.updateSessionUsage:145": "更新会话的 token 用量统计",
	"src/server/session-db.ts/SessionDB.getMessages:156": "获取指定会话的所有消息",
	"src/server/session-db.ts/SessionDB.getMessagesWithSeq:163": "获取指定会话带序号的消息列表",
	"src/server/session-db.ts/SessionDB.getMessageCount:169": "获取指定会话的消息数量",
	"src/server/session-db.ts/SessionDB.saveTurn:176": "保存一轮完整消息并更新会话时间戳",
	"src/server/session-db.ts/SessionDB.deleteMessage:203": "删除指定会话中特定序号的消息",
	...
	"src/server/session-db.ts/SessionDB.createTurnState:318": "创建轮次状态检查点",
	"src/server/session-db.ts/SessionDB.updateTurnPhase:330": "更新轮次执行阶段和检查点",
	"src/server/session-db.ts/SessionDB.completeTurnState:342": "标记轮次状态为已完成",
	"src/server/session-db.ts/SessionDB.failTurnState:354": "标记轮次状态为失败并记录错误",
	"src/server/session-db.ts/SessionDB.getIncompleteTurns:366": "查询所有未完成的轮次状态",
	"src/server/session-db.ts/SessionDB.cleanOldTurnState:384": "清理超过指定时间的旧轮次状态",
	"src/server/session-db.ts/SessionDB.deleteTurnState:402": "删除指定会话的所有轮次状态",
	"src/server/session-db.ts/SessionDB.close:410": "关闭数据库连接",
	...
	// ── server/kb-store.ts ──
	"src/server/kb-store.ts/KbStore.constructor:27": "初始化知识库存储",
	"src/server/kb-store.ts/KbStore.list:31": "列出所有知识库",
	"src/server/kb-store.ts/KbStore.get:35": "按 ID 获取知识库",
	"src/server/kb-store.ts/KbStore.create:39": "创建新知识库",
	"src/server/kb-store.ts/KbStore.update:43": "更新知识库配置",
	"src/server/kb-store.ts/KbStore.delete:47": "删除知识库",
	"src/server/kb-store.ts/KbStore.updateFile:51": "更新知识库中的文件信息",
	"src/server/kb-store.ts/KbStore.removeFile:63": "从知识库中移除文件记录",
	// ── server/kb-db.ts ──
	"src/server/kb-db.ts/KbDB.constructor:24": "初始化知识库数据库并创建表结构",
	"src/server/kb-db.ts/KbDB.insertChunk:51": "插入单个文本块及其向量嵌入",
	"src/server/kb-db.ts/KbDB.insertChunksBatch:60": "批量插入文本块及其向量嵌入",
	"src/server/kb-db.ts/KbDB.deleteFileChunks:74": "删除指定文件的所有文本块",
	"src/server/kb-db.ts/KbDB.deleteKbChunks:78": "删除指定知识库的所有文本块",
	"src/server/kb-db.ts/KbDB.getChunkCount:82": "获取知识库的文本块数量",
	"src/server/kb-db.ts/KbDB.getFileChunkCount:87": "获取知识库中指定文件的文本块数量",
	"src/server/kb-db.ts/KbDB.getAllChunksForSearch:92": "获取知识库中所有含嵌入向量的文本块",
	"src/server/kb-db.ts/KbDB.getFileList:98": "获取知识库中按文件分组的块数量列表",
	"src/server/kb-db.ts/KbDB.close:104": "关闭数据库连接",
	// ── server/kb-search.ts ──
	"src/server/kb-search.ts/search:15": "对查询文本做向量嵌入后检索最相关的文本块",
	"src/server/kb-search.ts/formatSearchResults:53": "将搜索结果格式化为带引用编号的 Markdown 文本",
	// ── server/kb-embeddings.ts ──
	"src/server/kb-embeddings.ts/OpenAIEmbeddingProvider.constructor:18": "初始化 OpenAI 兼容的嵌入提供者",
	"src/server/kb-embeddings.ts/OpenAIEmbeddingProvider.embed:25": "调用 OpenAI 接口生成文本嵌入向量",
	"src/server/kb-embeddings.ts/OllamaEmbeddingProvider.constructor:57": "初始化 Ollama 嵌入提供者",
	"src/server/kb-embeddings.ts/OllamaEmbeddingProvider.embed:63": "逐条调用 Ollama 接口生成嵌入向量",
	"src/server/kb-embeddings.ts/createEmbeddingProvider:84": "根据提供者类型创建对应的嵌入服务实例",
	// ── server/kb-router.ts ──
	"src/server/kb-router.ts/createKbRouter:12": "创建知识库 API 路由",
	// ── server/kb-ingest.ts ──
	"src/server/kb-ingest.ts/ingestFile:22": "读取文件内容并分块嵌入后存入知识库",
	"src/server/kb-ingest.ts/removeFile:103": "从知识库中删除指定文件的所有文本块",
	// ── server/stores ──
	...
	"src/main/ipc/kb-handlers.ts/registerKbHandlers:6": "注册知识库 CRUD、文件导入和搜索的 IPC 处理器",
	...
	"src/shared/github-template-utils.ts/shouldSkipMd:24": "判断 Markdown 文件是否应被模板扫描跳过",
		...
		"src/runtime/tools/index.ts/isEnabled:128": "检查指定工具在当前配置下是否启用",
		"src/runtime/tools/outline/extractors/json.ts/JsonExtractor.extract:4": "解析 JSON 文件结构并提取大纲节点",
		"src/runtime/tools/outline/extractors/json.ts/JsonExtractor.extractValue:15": "提取 JSON 值的内容摘要",
		"src/runtime/tools/outline/extractors/json.ts/JsonExtractor.extractObject:25": "递归提取 JSON 对象的键值对大纲",
		"src/runtime/tools/outline/extractors/json.ts/JsonExtractor.extractArray:66": "提取 JSON 数组的元素概览",
		"src/runtime/tools/outline/extractors/json.ts/JsonExtractor.findKeyLocation:113": "在文本中定位 JSON 键的位置",
		"src/runtime/tools/outline/extractors/json.ts/JsonExtractor.findValueEnd:122": "找到 JSON 值的结束位置",
		"src/runtime/tools/outline/extractors/json.ts/JsonExtractor.findArrayItemLocation:141": "定位数组中指定索引的元素位置",
		"src/runtime/tools/outline/extractors/json.ts/JsonExtractor.fallbackExtract:156": "无法解析时按行生成基本大纲",
		"src/runtime/tools/outline/extractors/markdown.ts/MarkdownExtractor.extract:4": "解析 Markdown 文件提取标题层级",
		"src/runtime/tools/outline/extractors/markdown.ts/MarkdownExtractor.parseHeadings:9": "逐行扫描 Markdown 标题并构建节点树",
		"src/runtime/tools/outline/extractors/python.ts/PythonExtractor.extract:4": "解析 Python 文件提取类和函数定义",
		"src/runtime/tools/outline/extractors/python.ts/PythonExtractor.parseBlock:9": "解析单个缩进块的子节点",
		"src/runtime/tools/outline/extractors/python.ts/PythonExtractor.findBlockEnd:105": "根据缩进级别确定块的结束行",
		"src/runtime/tools/outline/extractors/python.ts/PythonExtractor.getIndent:115": "计算行的前导空格数",
		"src/runtime/tools/outline/extractors/typescript.ts/TypeScriptExtractor.extract:5": "解析 TypeScript 文件提取声明和结构",
		"src/runtime/tools/outline/extractors/typescript.ts/TypeScriptExtractor.tryParseDecl:41": "尝试解析单行声明语句",
		"src/runtime/tools/outline/extractors/typescript.ts/TypeScriptExtractor.findBlock:86": "根据花括号匹配查找代码块范围",
		"src/runtime/tools/outline/extractors/typescript.ts/TypeScriptExtractor.isKeyword:212": "判断标识符是否为 TypeScript 关键字",
		"src/runtime/tools/outline/extractors/typescript.ts/TypeScriptExtractor.summary:216": "生成文件级别的摘要信息",
		"src/runtime/tools/outline/index.ts/C_EXT:33": "C 语言文件扩展名常量列表",
		"src/runtime/tools/outline/index.ts/CPP_EXT:34": "C++ 文件扩展名常量列表",
		"src/runtime/tools/task-status.ts/clip:6": "将文本截断到指定最大长度",
		...
		"src/server/kb-db.ts/KbDB.initSchema:34": "创建知识库数据库的表结构",
		"src/server/kb-ingest.ts/extractPdfText:111": "从 PDF 文件中提取纯文本内容",
		"src/server/kb-ingest.ts/splitIntoChunks:131": "将文本按固定大小分块并保留重叠",
		"src/server/kb-search.ts/cosineSimilarity:70": "计算两个向量的余弦相似度",
		...
		"src/server/session-db.ts/SessionDB.initSchema:54": "创建会话数据库的表和索引",
		"src/server/session-db.ts/SessionDB.migrateMessageFiles:231": "从旧格式 JSON 文件迁移消息数据",
		"src/server/session-db.ts/SessionDB.rowToRecord:418": "将数据库行转换为业务记录对象",

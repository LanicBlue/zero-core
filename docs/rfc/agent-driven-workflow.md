...
**沉淀是层级内、私有的。** 每个 agent 积累的是**自己的**工具与环境使用经验(调查 agent 沉淀「这个项目 grep 什么最快能定位鉴权」、测试 agent 沉淀「这个 fixture 要先 build 才生效」),服务于**自己下次干得更好**,不需要也不告诉其他 agent。跨 agent 传递的只有「结果」(archivist 的 wiki、需求文档、交付的代码),没有「我怎么得出的」。这与上面的目的同向:经验私有积累让 agent 在自己这一层不必反复摸索,视角更干净。
...
- **门 = 工具调用本身,不是外加的拦截层**(v0.6):plan 门、build 的每步分派门、verify 门都是 lead 调用对应工具(plan / execute / verify)时**该工具内部自带的门控** —— 工具一被调用即暂停等审核,不通过则工具返回 `false` + 理由(lead 据此自重或回退),通过则工具继续执行并推进状态。这跟「工具即 agent 间接口」的哲学一致:门是 lead 在自己管线里调工具时自然发生的,不需要外部 hook 拦截。requirement 状态(`plan`→`build`→`verify`)由工具调用的成功推进,自治 agent 不在 pending 空转。
...
**(b) verify 标准化为证据清单 —— 给 PM 验收用,不引入多门禁聚合。**
需求进 `verify` 时,挂一份**证据清单**而不是让 PM 拍脑袋 accept/reject:
- 需求文档的**验收标准逐条**核对(每条 pass / unmet)
- **单测**是否覆盖了本次改动(needs-evidence → pass)
- **smoke** 是否跑通(needs-evidence → pass)
- archivist 的 **traceability** 是否接上(本次改动 ↔ 需求文档,见 §2.13)

PM 验收 = 看这份清单从 `needs-evidence` 变 `pass`;有 `unmet` / `needs-evidence` 则 reject 并附清单给 lead 修。**不抄 openprd 的 `productionReady` 多门禁聚合**(traceability/redaction/performance 那一套是通用 harness 对任意项目的诉求,我们一个具体内嵌工作流不需要那套复杂度,只保留直接服务验收的几条)。
...
### 4.4 ProjectWikiNode(改挂 projectId + traceability + provenance)

`projectId` 指向 Project;新增 `requirementIds: string[]`(该节点关联的需求,archivist 维护);新增断言级 `provenance: "structure" | "derived" | "confirmed"`(archivist 自己判断该信哪条,见 §2.17a)。
...
### 数据层
- `src/shared/types.ts` — AgentRecord +`projectId`/`cron`/`lastScannedRef`;`ProjectRecord` 精简;RequirementRecord +`docPath`/指派/verify 证据清单;ProjectWikiNode +`requirementIds`+`provenance`
...
- **新增 `archivist-service.ts`** — git 增量扫描、wiki 维护(+ node provenance 打标)、traceability 拼接、**文档说明书反向维护**(扫到文件实质变化时回写 header/folder README/docs-basic,仅注释区,逻辑 read-only 守卫)、**docs/basic↔wiki 分歧检测**
...
- `src/runtime/tools/orchestrate-tool.ts` — plan/execute 分离,不再造临时 role agent;**plan / execute / verify 工具内置门控**(调用即暂停等审核,不通过返回 false+理由,见 §2.9 / §2.17b)
...
11. ✅ 门 = 工具调用本身(v0.6 修订):plan/execute/verify 工具内置门控,调用即暂停等审核、不通过返回 false+理由,不另设 hook 拦截层;驳回回路不变(plan 驳回 lead 自重 / verify 驳回通知 lead 修 / 根本问题退 discuss)
...
**设计哲学与收窄(v0.6 新增)**
32. ✅ 工作流目的 = 层级调用给每个 agent 一个干净视角(每层只处理自己颗粒度的问题,细节委托下层专门 agent);沉淀 = 层级内、私有(agent 自己的工具/环境经验,服务自己下次,不跨 agent)
33. ✅ wiki node provenance(`structure`/`derived`/`confirmed`)只给 archivist 自己判断该信哪条 + PM/lead 读时参考,不泛化成全工作流标签
34. ✅ verify = 证据清单(验收条目逐条 + 单测/smoke/traceability 的 needs-evidence→pass),PM 据此验收而非拍脑袋;**不引入** openprd productionReady 多门禁聚合(通用 harness 的复杂度,本工作流不需要)
...
本 RFC 定稿后,再起独立 milestone 计划(plan/实现),按数据模型与影响清单执行。v0.6 把设计哲学显式化、三处机制收窄进 §2.17(决策 32-35);实现期需重点设计:
- plan/execute/verify **工具内置门控**(调用即暂停、返回 false+理由的状态机)
- wiki node **provenance 打标**(archivist 推断 vs 人确认)
- verify **证据清单**的存储与 PM 验收 UI
其余(`path.resolve+realpath` 落库、子 session 双 API 开关、archivist 仅注释区写入守卫、docs/basic↔wiki diff 检测器等)均为纯实现期细节,按决议执行即可。

...
- **对代码 read-only**(不改项目代码);周期扫描代码库 → 更新 wiki 知识图谱(架构、模块、依赖、约定)
- **兼管 main 分支 git**:统一 commit PM 文档/自己 wiki、verify 后合并 feature→main、清理 worktree(见 §2.15);feature 分支 git 归 lead
- **PM 和 lead 都读它**:PM 读 wiki 写好需求,lead 读 wiki 做好 plan
- 有自己的 chat 页面,**可与用户直接对话**(解释架构、澄清设计),但 PM 是默认入口,一般不直接打扰用户
- 详见 §2.13 archivist 的更新与意图理解机制
...
- **写隔离靠角色 scope,不用锁**:
  - PM cron:**只建新需求文档**,wiki/代码 read-only
  - PM discuss:**只改已有需求文档**(可能写 wiki)
  - archivist:**只写 wiki**,代码 read-only
  - → cron 与 discuss 写的是**不相交的目标**,无冲突;wiki 写入者收敛到 archivist + discuss-PM,偶发重叠 last-write-wins + git 兜底
...
| archivist | 项目绑定 | 持 cron,代码 read-only,toolPolicy 放行 analyzer(architecture lens) | 知识:wiki + 需求↔设计 traceability |
...
- **新增 `archivist-service.ts`** — git 增量扫描、wiki 维护、traceability 拼接
...
本 RFC 定稿后,再起独立 milestone 计划(plan/实现),按数据模型与影响清单执行。剩余 Q4 / Q6 残留 / Q-字段共存 三个小问题可在实现期收口。

...
- **engine 是 target 属性,接口层之下 dispatch**:resolveTarget(id) → {engine: "internal"|"claude"|"codex", …}。所有 engine 分支(delegate、resume、kill、重启恢复)都在 delegator 层以下,LLM 看到的 API 零 engine 分支。
...
```
                parent AgentLoop (内部)
                       │ delegateTask(target=external:claude)
                       ▼
            ┌──────────────────────┐
            │  subagent-delegator  │  ① mint scope token T → in-mem map
            │  (external 分支)     │  ② insert delegated_tasks(engine='claude')
...
- `DelegateTaskOptions` 增 `engine?: "internal" | "claude" | "codex"`(默认 `internal`,现状不变)。
- `delegateTask`:if `engine !== "internal"` → 走**新分支**:
  1. mint token(写 tokenStore,scope 由 callerBundle.projectId + readOnly/allowedTools 推导)。
  2. insert `delegated_tasks`(新增 `engine` 列;`target_agent_id` 存 `external:<kind>`;无 session_id/owner_session —— 外部进程非 AgentLoop)。
...
- 外部 agent **不**经 `engine` 入参暴露给 LLM。改为:已配置+启用+测试通过的外部 agent(见组件 7)作为 **target** 出现在委派的 Target agent 选择器里(与内部 agent 同列),`targetAgentId` 取值 `external:claude` / `external:codex`。
- 现有 `agent`/delegate 工具(`src/tools/agent.ts`)的 target 解析路径扩展:`resolveAgent("external:claude")` → 命中外部 target → delegator 走外部分支(组件 4)。内部 target 路径不变。
...
## 决策点(design→plan gate — 已全部敲定 2026-07-09)

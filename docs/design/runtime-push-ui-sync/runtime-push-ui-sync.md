...
- `model / provider` —— **已补(N4)**。每轮已被重读([agent-loop.ts:577](../../../src/runtime/agent-loop.ts#L577) `resolveModel`),只差没被写回;N4 已补进 `applyConfigUpdate`,下轮生效(轮间切模型安全)。
- `thinkingLevel` —— **已补(N4)**。PreLLMCall 的 provider-options hook(`provider-options-hooks.ts`)每轮重读 `ctx.config.thinkingLevel`,N4 已补进 `applyConfigUpdate`,下轮 providerOptions 自动用新。
- `contextConfig`(device/guidelines/memory)—— **经核实为 Electron 路径死字段**:DB 存(`agents.context_config`)、UI 可编辑,但 runtime/server **零消费**(建 loop 时未解析进 prompt/contextBundle)。仅 headless CLI 路径 `cli.ts:196` 的 `buildSystemPrompt` 用 `contextConfig/enabledSkills`。当前不构成『所见≠所跑』冲突(无『所跑』)。接通它需先在建 loop 阶段把 device/guidelines/memory 解析进 systemPrompt/contextBundle + 改 applyConfigUpdate re-resolve + `invalidate("base")`,是**单独的功能开发**,不在本努力范围。
- `skillPolicy`(enabledSkills,影响工具集)/ `knowledgeBaseIds`(影响 context)—— 同上,**经核实为 Electron 路径死字段**:DB 存、UI 可编辑,但 runtime/server 零消费;仅 headless CLI 的 `buildSystemPrompt` 读 `enabledSkills`。当前不构成『所见≠所跑』冲突(无『所跑』)。接通属单独功能开发,不在本努力范围。
- 注:`maxTokens`/`temperature` 非 AgentRecord 字段,非 gap。
- 注:gap 仅在"loop 忙时改"出现(空闲时 `store.onChange` 整个重建 loop,全字段生效)。
**待办(N4 已完成 model/provider/thinkingLevel 三字段)**;contextConfig/skillPolicy/knowledgeBaseIds 不在本节点接通(死字段,见上)。
...
| 节点 | 主题 | 依赖 | 实现路线 | 测试要求 |
|---|---|---|---|---|
| **N1** | 统一状态流基建(桥 + runtime emit + 白名单 + session emit) | 无(基石) | [plan-N1.md](../../plan/runtime-push-ui-sync/plan-N1.md) | [acceptance-N1.md](../../plan/runtime-push-ui-sync/acceptance-N1.md) |
| **N2** | UI 推送驱动 + 消闪烁 + 重连 resync | N1 | [plan-N2.md](../../plan/runtime-push-ui-sync/plan-N2.md) | [acceptance-N2.md](../../plan/runtime-push-ui-sync/acceptance-N2.md) |
| **N3** | 文件系统零轮询(非运行时) | 无 | [plan-N3.md](../../plan/runtime-push-ui-sync/plan-N3.md) | [acceptance-N3.md](../../plan/runtime-push-ui-sync/acceptance-N3.md) |
| **N4** | 配置字段热更(不变量 1) | 无 | [plan-N4.md](../../plan/runtime-push-ui-sync/plan-N4.md) | [acceptance-N4.md](../../plan/runtime-push-ui-sync/acceptance-N4.md) |
...
- **[已定案,实现时收窄] applyConfigUpdate 字段补全**:核实后,**仅 model / provider / thinkingLevel 三个字段真正构成『所见≠所跑』缺口**(model/provider 每轮已被 `resolveModel` 重读、只差写回;thinkingLevel 每轮被 PreLLMCall provider-options hook 重读)——N4 已补全这三个,每轮重读故无需缓存失效。原列的 `contextConfig / skillPolicy / knowledgeBaseIds` 经审计为 **Electron 路径死字段**(DB 存 + UI 可编辑,但 runtime/server 零消费;仅 headless CLI 路径 `cli.ts:196` 的 `buildSystemPrompt` 读 `contextConfig/enabledSkills`),当前不构成『所见≠所跑』冲突(无『所跑』),接通它们属单独功能开发、不在本努力范围。maxTokens/temperature 非 AgentRecord 字段、非 gap。

...
#### `AgentRegistry`（agent 记录 CRUD + template 参考）
> 名字叫 `AgentRegistry` 而非字面 `Agent`——避开既有「子 agent 委派」工具 `Agent`（`runtime/tools/agent.ts` 的 delegateTool，用户可见、toolPolicy.tools['Agent'] 引用）。功能化命名（管 agent 记录），符合 §7.3 按功能命名原则。
| action | 字段 | 说明 |
|---|---|---|

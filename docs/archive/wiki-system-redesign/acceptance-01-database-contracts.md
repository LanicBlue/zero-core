...
- [ ] 临时 profile fresh open 后只在 `${ZERO_CORE_DIR}/db/wiki.db` 创建 Wiki DB/WAL/SHM，不修改 `core.db`。
...
- [ ] active `path` 与 `(parent_id,name)` partial unique 均有失败测试；归档后可创建同路径 active 节点，restore 冲突被拒绝。
- [ ] 删除被 link target/source binding 引用的节点符合 RESTRICT/CASCADE 设计。
- [ ] FTS 只含 `name/summary/content`，显式 transaction 的 insert/update/delete 与 rebuild 后结果一致；不存在 Wiki FTS trigger。
- [ ] audit `request_id` 重复不会产生两条记录。
- [ ] 对 Agent/UI 暴露的共享 view 不含内部 `id/parent_id/source_id/target_id`；mutation 可返回 opaque `auditId`。
- [ ] `PRAGMA table_info` 证明 `id/parent_id/revision` 等整数列全部为 INTEGER affinity，revision 算术不会字符串拼接。
- [ ] v1 kind 与 WikiErrorCode 精确等于 Plan 01 闭集，后续模块从同一共享定义 import。

## B. 结构审查

- [ ] 新 Wiki schema 不位于 `CoreDatabase` 或 `db-migration.ts` 的旧 `project_wiki` migration 中。
- [ ] Wiki schema/repository 未复用 `SqliteStore<T>`，DDL 使用明确 SQL。
...
- `PRAGMA table_info` affinity 表与 partial index SQL。
...
- Wiki 表由 `SqliteStore<T>` 创建导致 TEXT affinity。

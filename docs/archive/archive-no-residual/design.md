...
`archiveDelegatedSession` 只在 [createLoopForSession:1385](../../src/server/agent-service.ts#L1385) 接线;[sendProjectPrompt lazy-rebuild:1654](../../src/server/agent-service.ts#L1654) **抄漏** → 该 loop 派的子 agent 终态时 `onTaskTerminal=undefined` → [delegator:261](../../src/runtime/subagent-delegator.ts#L261) 早退 → 永不归档(264 行累积根因)。

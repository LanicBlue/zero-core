...
用户设计后两点已落地于 `archive-service.ts`:

| 用户设计点 | 现状 | 位置 |
|---|---|---|
| ② 先标记归档 → 调 LLM → 完成后 JSON 出表 | `archiveSession`:`markArchivedTransient` → memory ephemeral turn → 原子 export(tmp→parse→rename→`deleteSessionData`) | `archive-service.ts:327-430` |
| ③ 启动恢复被打断的归档 | `recoverInterruptedArchives(db)`,`index.ts` 启动扫 `archived=1 且仍有行` 重 export | `archive-service.ts:599` |
...
- `archiveDelegatedSession:987-989` 归档时要从行里读 `sessionId`/`targetAgentId`/`modelId`;
- `fireOnTaskTerminal:262` 已先 `getDelegatedTask(taskId).sessionId` 拿到 childSessionId。
...
### D2. Gap A:terminal 归档触发接线补全(已决策:B2 收口)

`archiveDelegatedSession` 只在 `createLoopForSession:1385` 接线;`sendProjectPrompt lazy-rebuild:1654` **抄漏** → 该 loop 派的子 agent 终态时 `onTaskTerminal=undefined` → `delegator:261` 早退 → 永不归档(264 行累积根因)。

**根因不是「漏一行」,是「建 loop 的代码重复了两份」**:createLoopForSession 与 sendProjectPrompt 各自内联了 `new AgentLoop` + `registerHooksForLoop` + `fireSessionStart`,sendProjectPrompt 那份少了 `archiveDelegatedSession`。session 就是 session,消息从 user 来还是 project work 来本质一样(同一 sessionId 的 loop 还复用),归档不该按入口区分——现在的区分是抄漏造成的 bug。

**B2(收口,已选)**:抽共享 `buildAndRegisterLoop(sessionConfig, agentId, sessionId)`,内部统一做:设 `sessionConfig.archiveDelegatedSession`(**须在 `new AgentLoop` 之前**,因 `agent-loop.ts:340` 构造时读它赋给 delegator 的 `onTaskTerminal`)→ `new AgentLoop` → `registerHooksForLoop(...,"main",...)` → `this.loops.set` → `fireSessionStart`。createLoopForSession 与 sendProjectPrompt 各自拼好 sessionConfig(chat 版 / work 版,这部分本就不同),再都调这个共享尾巴。wiring 写一次,**物理上不可能再分叉**。

放弃 B1(镜像补一行):治标——两份重复的建 loop 代码还在,下次再加 wiring 还会一边有一边没,正是本次 bug 的温床。
...
**实现陷阱(对抗式 review 发现)**:不能走 kill。`fireOnTaskTerminal` 只对 `completed`/`failed` 触发归档——`delegator:250` 明确 `killed` **被排除**(killed 设计为 abandoned、父 owns cleanup、不归档)。所以「kill 仍在跑的子任务」走 kill→terminal **不会**归档子 session,反造孤儿,与零遗留矛盾。

**正确做法:直接 archive 子 session**,不走 kill:

1. `archiveSession` 入口加一步 `await archiveChildrenOf(sessionId, db)`(teardown 之前);
2. 内部 `listDelegatedTasks({parentSessionId: 父})`(`session-db.ts:1708` 查询能力已有)拿所有子任务;
...
2. **sub-2(D2)**:抽 `buildAndRegisterLoop` 共享建 loop + 接线,createLoopForSession / sendProjectPrompt 共用。验收:两处建出的 loop 都带 `archiveDelegatedSession`(源码 grep + 集成测试:sendProjectPrompt loop 派子终态后归档真触发)。
...
| **D2** Gap A 接线 | **B2 收口共享建 loop** | 根因是建 loop 代码重复两份、sendProjectPrompt 抄漏;抽 `buildAndRegisterLoop`,wiring 写一次防分叉 |

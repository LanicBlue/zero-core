...
1. **search 不搜正文**:目前只匹配 title / summary / path 三个元数据字段([wiki-tool.ts:660](../../../src/tools/wiki-tool.ts)),节点正文(doc body)不在检索范围——某词只出现在正文深处就搜不到。正文存在(`docRead` 能读,`readNodeDetail` [wiki-node-store.ts:807](../../../src/server/wiki-node-store.ts)),只是没纳入 search。
...
| 脏值 | 来源 | 性质 |
|---|---|---|
| `project:<uuid>` | `projectSubtreeRootPath` [wiki-node-store.ts:121](../../../src/server/wiki-node-store.ts) `return \`project:${projectId}\`` | 代码 |
...
- **type 派生靠 path 前缀**:`deriveTypeFromPosition` [wiki-node-store.ts:1913](../../../src/server/wiki-node-store.ts) 用 `path.startsWith("intent:"/"header:"/"memory")` 分类——但是**多路分层回退**(id 前缀 `wiki-root:` → project、path 前缀、nodeType、projectId),path 前缀**大部分冗余**;唯 **memory 类型强依赖 `path.startsWith("memory")`**,删前缀要给它换信号(看 parent 是否 memory root)。
...
[wiki-tool.ts](../../../src/tools/wiki-tool.ts)(search 重写 + 寻址统一)、[wiki-node-store.ts](../../../src/server/wiki-node-store.ts)(`projectSubtreeRootPath` / `synthesizePath` / `deriveTypeFromPosition` / slug)、可能涉及存量数据迁移(`~/.zero-core` 下 6687 个 wiki 文件)。

...
- **reactive 集成测(sub-5 Lens B 移交)**:加一个真 `AgentLoop` 注册了 compression trigger hooks、驱动 `prompt_too_long` OnLLMError 的集成测,断言 compressSession 真被调 + summary 增。sub-5 的 unit 测用 runtimeShape 模拟 ctx 形态,但没自动测抓"生产 fire 真带 config/providers"的未来回归——本集成测补这个自动化缺口。

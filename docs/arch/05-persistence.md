...
```
~/.zero-core/
├── sessions.db            ← 主数据库 (SessionDB,better-sqlite3,§2 全部表 + memory_* 4 张自建表)
...
>
> **关于"30 张表"的口径**:这个数字严格指 **`db-migration.ts` 管理的表**(对应 §4.2 迁移机制的
> 5 阶段)。`sessions.db` 物理上还多 4 张表(`memory_nodes` / `memory_subjects` /
> `memory_edges` / `memory_nodes_fts`),它们由 `MemoryNodeStore.init()` 构造自建,**不进
> db-migration**(§2.2 末有专门说明块,06 §2.7 有横向矩阵)。此外 `kb_chunks` 表**根本不在
> sessions.db**,而在独立的 `knowledge.db`(§2.3),不要把它算进 sessions.db 的表数。
...
> ⚠️ **构造自建表(memory_* 4 张)不进 db-migration**。上表里的 `memory_nodes` / `memory_subjects`
> / `memory_edges` 加上 §2.5 的 `memory_nodes_fts`(共 4 张)都在
> `MemoryNodeStore.init()`(`memory-node-store.ts:138-184`)构造时 `CREATE TABLE IF NOT EXISTS`
> 自建 —— **`grep "memory_nodes" db-migration.ts` 零命中验证**。所以本节标题"30 张表"严格说只数
> **db-migration.ts 管理的表**,sessions.db 实际物理表数 > 30(再加这 4 张)。改 memory_* schema
> 要去 `memory-node-store.ts` 的 `init()`,不要去 `db-migration.ts`(那会漏掉 FTS 重建逻辑:
> `init()` 检测到旧 FTS 列结构时会 DROP 重建,db-migration 没这个能力)。详见 06 §2.7
> "三套知识系统对比矩阵"的"建表机制"列。

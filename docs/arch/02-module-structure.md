...
| ~~context-manager.ts~~ | ~~295~~ | ⚠️ **已删**：token 估算 + 修剪/压缩已迁至 `runtime/session.ts` + `server/compression-core.ts`（由 `runtime/hooks/compression-trigger-hooks.ts` 触发） |
...
- **实际例外** —— `test-seed.ts` 是这条边界的**两处轻微泄漏**,但是测试辅助代码:
  - **类型层**:import type 多个 `../server/*-store.js`(`CoreDatabase`/`AgentStore`/`ProviderStore`/`ProjectStore`),编译时擦除,**不产生运行时依赖边**。
...
├── turn-hooks.ts                  # TurnStart/StepEnd/PostToolUse/Failure/TurnEnd/TurnError → steps 表 step 级持久化（原 turns 表已 rename 为 steps）
...
| wiki/ | 子目录,20 文件 | **Wiki v2 子系统**(plan-01..08 cutover 后的当前实现):`WikiDatabase` / `WikiService` / 各 repository / `compileWikiAccess` / `compileWikiContext` / `WikiProjectIndexer`,详见 [06 §0](./06-knowledge-subsystems.md#0-wiki-v2plan-01plan-08-cutover-后的当前模型) |
| wiki-backup-service.ts | n/a | `BackupService`:SQLite Backup API 单一所有者(plan-08 §3) |
| wiki-skeleton-service.ts | n/a | `WikiSkeletonService`:orchestrator facade(P1-6 后 vestigial stub 已清,委托给 `WikiProjectIndexer`) |
| wiki-router.ts / wiki-admin-router.ts / wiki-maintenance-router.ts | n/a | 数据面 / 管理面 / 维护面 HTTP router |
...
> **v0.8 关键更正**：早期文档把 `core-database.ts` 描述成"所有 store 的聚合门面"——**已不准确**。v0.8 后 CoreDatabase 只聚合 2 个 eager 内核 store（`KeyValueStore`/`MemoryNodeStore`；原 3 个里的 `MemoryStore` 已作为僵尸清理删除）+ 2 个 M5 lazy store（`ExtractionCursorStore`/`TelemetryStore`），共 4 个；而工作流域 store（`ProjectStore`/`RequirementStore`/`CronStore`/`TaskStepStore`/`ProjectJobStore`/`CronRunStore`/...）在 `index.ts:151-171` **独立 new**，只把 CoreDatabase 当 `getDb()` 提供者。详见 [05 §4.0.3](./05-persistence.md#403-关键边界sessiondb-不是聚合根) 与下方 §4.1.1。

> **wiki-system-redesign cutover 后**：原 `wiki-node-store.ts` / `project-wiki-store.ts` / `wiki-scan-cursor-store.ts` 三个 v0.8 store 文件已**物理删除**(plan-08 §1),其职责由 `src/server/wiki/` 子目录下的 `WikiService` + 各 repository 承担。原 `project_wiki` 表也 DROP,新 schema 是独立 `wiki.db` 的 7 张表 + FTS5(详见 [06 §0](./06-knowledge-subsystems.md#0-wiki-v2plan-01plan-08-cutover-后的当前模型))。下文 §4.1.C 旧表中相关行已移除。

**架构判断**：服务层的复杂度正在向**组合根（store 编排）**、Agent 编排、Wiki v2 子系统、迁移脚本四个热点集中。后续拆分不应只盯 `AgentService`，也要把 `db-migration` 以及 `index.ts` 内 12+ store 的手动编排（应引入 store registry）纳入计划。Wiki v2 因已独立为 `src/server/wiki/` 子目录(20 文件) + 独立 `wiki.db`,其内部演进不再外溢到 `server/` 顶层。
...
> **切分视角说明**：这里的 A/B/C/D 是按**归属方式**（是否挂 CoreDatabase + v0.8 阶段）切的；[`file-structure.md`](../basic/file-structure.md) 的 server 章节（§"数据存储"）则按**业务域**切（会话核心 / 旧业务实体 / 工作流域，再分路由层 + 服务编排）。两种切分是**正交**的，不强行统一：本节关心"谁持有 store 引用、store 怎么被 new 出来"，`file-structure.md` 关心"store 属于哪个业务域、跟哪些 router/service 配套"。Wiki v2 子系统因 cutover 后已迁入 `src/server/wiki/` 子目录(独立 `wiki.db`),不参与 CoreDatabase 编排,既不属 A/B/C/D 任一类,详见 [06 §0](./06-knowledge-subsystems.md#0-wiki-v2plan-01plan-08-cutover-后的当前模型)。
...
**C. v0.8 工作流域 store**（`index.ts:159-171` 独立 new，**不挂 CoreDatabase**）—— 详见 [05 §2.2b](./05-persistence.md#22b-v08-多-agent-工作流域表)：
...
> **C 类 wiki store 已删除**(plan-08 cutover):原 `wiki-node-store.ts` WikiStore(1698 行) / `project-wiki-store.ts` ProjectWikiStore / `wiki-scan-cursor-store.ts` WikiScanCursorStore(62 行) 三个文件 + 对应 `project_wiki` / `wiki_scan_cursors` 表已整体物理删除。Wiki v2 走独立 `wiki.db`,store 层是 `src/server/wiki/` 子目录下的 `WikiService` + `wiki-{node,link,audit,repository}-repository.ts`(详见 [06 §0](./06-knowledge-subsystems.md#0-wiki-v2plan-01plan-08-cutover-后的当前模型))。cutover 前的 wiki 编辑/扫描路径不再存在。
...
> Wiki v2 子系统(`src/server/wiki/` 20 文件 + 顶层 `wiki-{router,admin-router,maintenance-router,operations,skeleton-service,backup-service}.ts`)使用独立 `wiki.db`,**不参与 CoreDatabase 编排**,store/repository 由 `WikiService` 内部组装。见 [06 §0](./06-knowledge-subsystems.md#0-wiki-v2plan-01plan-08-cutover-后的当前模型)。
...
const databaseManager = new DatabaseManager(...); // 持有 core.db + wiki.db 生命周期(plan-00 §3)
const wikiDatabase = databaseManager.getWikiDatabase(); // 独立 wiki.db(WikiDatabase,plan-01 §1)
const wikiService = new WikiService(wikiDatabase, ...); // 内部组装各 repository
// ...
...
server/index.ts 注入主要 HTTP 表面：
  ...
  文件/日志：/api/files, /api/logs
  Wiki v2（plan-06/07/08）：/api/wiki（数据面）, /api/wiki-admin（管理面）, /api/wiki-maintain（维护面）
  工作流：/api/projects, /api/requirements, /api/orchestrate, /api/pm, /api/crons, /api/archivist
```

> 历史 `/api/kb` / `/api/project-wiki` / `/api/memory-nodes`(Gen1) endpoint 均已随对应子系统退役删除。当前 wiki 唯一活跃 API 面是 `/api/wiki{,-admin,-maintain}` 三平面。
...
- `durable-hooks.ts`（105）— Hook 的"首位持久化消费者"，管理 sessions 表检查点（原 `turn_state` 表已 DROP，列并入 `sessions`）
...
- 启动顺序（`index.ts:113-150` 实际序列，比早期"CoreDatabase→runMigrations→...→Stores"线性模型复杂）：
  ...
  3. `new DatabaseManager(...)` —— 持有 core.db + wiki.db 双生命周期(plan-00 §3);末尾 `new WikiDatabase(wikiDbPath)` 完成 wiki.db open + 固定根 bootstrap(plan-01 §1)
  ...
  8. **手动 new 工作流域 store**（B + C 类，§4.1.1）—— 不挂 CoreDatabase，只在 `index.ts` 局部变量
  9. 各 `*-router(deps)` 注入 + `app.use(...)` —— 包括 wiki 数据/管理/维护三平面
  10. `startServer()`
  
  **关键约束**：wiki.db 必须在 wiki router / AgentService 装配前 ready(`DatabaseManager.open()` 同步完成);CronStore/ProjectStore 等工作域 store 之间无顺序依赖（互不引用）。
...
¹ runtime 原本设计为通过 `ISessionStore` / `IKVStore` 接口使用 server 实现；当前代码已出现多处 `runtime -> server` 的类型/常量 import（MemoryNodeInput、ManagementService、orchestrate-store 等）。这是边界侵蚀，不是理想依赖方向。Wiki v2 通过 `compileWikiContext` 的返回值(prompt 段落)+ `wikiAccess` snapshot 注入 CallerCtx,runtime 不直接 import `server/wiki/*`(memory feedback-agent-loop-hooks-only:wiki 走 PreLLMCall/PostTurnComplete hook,不内联 AgentLoop)。
...
graph TB
    ...
    subgraph "内核层"
        Core["core/<br/>config · hook-registry · logger<br/>system-prompt · tool-registry<br/>⚠ 不依赖上层"]
    end
    ...
    subgraph "服务层"
        Server["server/<br/>agent-service · core-database · wiki/ subdir<br/>20+ HTTP surfaces · mcp-manager"]
    end
...
4. `core-database.ts` 应逐步退化为 DB lifecycle + store factory；`db-migration.ts` 也需要拆出更小的 schema/domain/service 单元。Wiki v2 子系统(`src/server/wiki/`)已天然按 schema / repository / service / compiler / indexer 分文件,可作为这种拆分的参考样板。

...
- `runtime/hooks/extraction-hooks.ts`：StepEnd 后按阈值调度 Extractor A/B(hook-redesign Step 3A 从 PostTurnComplete 迁来),Extractor A 将结构化记忆写入 Wiki。
...
`runtime/hooks/rag-hooks.ts` 仍被 `registerHooksForLoop()`(per-loop)注册,但 hook 的第一步是:

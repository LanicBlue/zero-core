...
5. **核心大文件规模更新**：`agent-service.ts`、`db-migration.ts`、`core-database.ts`、`server/index.ts`、`agent-loop.ts` 均已进入大文件区间，架构债务不再只集中于 AgentService / CoreDatabase。(原 `wiki-node-store.ts` 已在 wiki-system-redesign cutover 中物理删除,wiki v2 拆为 `src/server/wiki/` 子目录下 20 个文件,见 [06 §0](./06-knowledge-subsystems.md#0-wiki-v2plan-01plan-08-cutover-后的当前模型)。)
6. **记忆主线已切到 Wiki v2**：当前默认 Agent 会话通过 `compileWikiContext` hook 注入项目/Agent 记忆(grants + context 编译到 CallerCtx);`rag-hooks.ts` 已随 KB 子系统整体退役(`getRagContext` 路径不再存在)。

...
> **TaskTree 数据源切换(live 内存 ⊕ DB 重启回填)**:TaskTreePanel 读 `runtimeTasks:bySession`(`GET /api/runtime-tasks/by-session/:sessionId`)→ `agentService.getRuntimeTaskTree(sessionId)`,现在是 **live TaskRegistry 与 `delegated_tasks` 表的并集**:无 live loop 时返回 DB 行(跨重启历史),live 存在时覆盖同 id 行 + 补 bash 后台任务。这样 UI 与 agent 的 TaskList **同源**,数量/状态不分叉,且重启不丢。`delegatedTasks:bySession`(纯 DB 视图)保留给历史检查;TaskDetailView 的下栏对话另用 `sessionsGetInit(delegatedSessionId)` 拉委派 session 的 turns。
>

...
> **TaskTree 数据源切换(live 内存)**:TaskTreePanel 改读 `runtimeTasks:bySession`(`GET /api/runtime-tasks/by-session/:sessionId`)—— 直接走 `agentService.getRuntimeTaskTree(sessionId)` → `loops.get(sessionId).getRuntimeTaskTree()`,即 `SubagentDelegator.taskRegistry` 的内存视图(含 bash 后台任务,带 `parentTaskId` 重建委派树)。这样 UI 与 agent 的 TaskList **同源**,数量/状态不再分叉。`delegatedTasks:bySession`(DB 视图)保留给历史/崩溃恢复检查,TaskTree 面板不再用它。完成确认生命周期:agent 调 `Agent action:'complete'` → `TaskRegistry.acknowledge` 把终态任务从内存 Map 移除 → 面板/TaskList 同步消失。

...
data-change-hub  (src/server/data-change-hub.ts)
   ├ 白名单 UI_COLLECTIONS = {agents, projects, crons, requirements}(cutover 后移除 project_wiki)
...
- **白名单**(`UI_COLLECTIONS`):`agents / projects / crons / requirements`(cutover 后 `project_wiki` 移除,wiki v2 走独立 pull-on-display,见顶部 banner)。`messages/turns/tool_usage` 等高频表不入(流式每 chunk 写 messages,会刷屏——这些走 `agent:event`)。
...
> 2. **`data:changed`(WS 反向,见 §2.3.1)** → 各 store **自己**在模块副作用里调 `data-sync.ts` 的 `subscribeDataChange` / `subscribeListDataChange` 订阅,与 AppLayout **完全无关**。这是 v0.8 工作流域 store 的主动同步机制,4 个 collection(`agents` / `projects` / `requirements` / `crons`)分别由对应 store 各自订阅(cutover 后 `project_wiki` 不再走这条路径,wiki v2 改为独立 pull-on-display,见顶部 banner)。

```mermaid
graph LR
    subgraph WS["Backend WebSocket 反向通道"]
        SE["StreamEvent<br/>(14 类)"]
        DC["data:changed<br/>(4 collection,cutover 后)"]
    end
    ...
    WikiV2Pull["Wiki v2 (pull-on-display)<br/>WikiTreePanel / WikiAccessSection"] -.-> WikiPage
    ...
    style WikiV2Pull fill:#93c5fd,color:#000

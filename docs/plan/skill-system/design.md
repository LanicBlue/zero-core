...
2. **重名处理 = 层级优先级覆盖**(协议默认 + **sub-12 产品决策反转**):协议默认 personal(`~/.claude`、`~/.agents`)> app/bundled(`~/.zero-core`)。**sub-12(2026-07 用户反馈)反转**——zero-core 产品选择:**自己的 skill 视为权威/精调,覆盖外部同名**(即 app/bundled 优先级最高)。理由:zero-core 自带 skill 是产品精调,优先级高于用户从 `~/.claude` 装的通用 skill。实现:scanner `getSkillRoots()` 把 `~/.zero-core/skills` 放数组**最后**(`scanSkills()` 后写入覆盖前写入 → app 胜)。`source` 字段仍按真实来源标记(app/user),不因此反转。
...
11. **agent 自建 skill**:per-agent `canAuthorSkills` 门禁(toggle 在 SkillsSection,默认 false);用基本 Write/Edit 经 `[skills]/` 虚拟路径写(**不造新工具**);仅落 `~/.zero-core/skills/`,拒写外部来源;id path-safe + 重名拒绝;frontmatter `author: agent:<id>` 溯源;无审批队列;prompt 文案克制引导。
    **sub-12 修订**:门禁从 `canAuthorSkills === true` 改为 `enabledSkills.includes("skill-creator")` —— agent 勾选 skill-creator(zero-core 自带 skill,见决策 2 sub-12 反转)即同时获得"使用 skill-creator"和"创建/编辑 skill"两项许可。`canAuthorSkills` 字段、SkillsSection toggle、prompt「Authoring Skills」引导段均移除;skill-creator 的 name+description 已在 Available Skills 列表(sub-4)触发 agent 经 progressive disclosure 读其正文获取引导。
...
9. **buildSystemPrompt 接入 agent runtime**(skills section)— plan 外补丁:发现 sub-4 的 prompt 注入是 dead path(buildSystemPrompt 仅供 cli/headless,agent 运行时用 agent.systemPrompt 不经它),sub-4/sub-8 的 skill 段运行时不生效。修法:assembleSystemPrompt 加 `skills` section(镜像 work-context 模式,cacheBreak:false + invalidate),抽 `buildSkillsSection()` 单一真理源,buildSystemPrompt + cli 加标注(防以后误解)。核心:skill 段真正进 agent prompt。
10. **SkillsSection UI 对齐 ToolsSection + scanner origin 字段**— 用户反馈:agent skill 配置页 UI 远差于工具配置页。改用 `toggle-switch`(替 checkbox)、可点展开详情、per-skill source badge;scanner 加 `origin` 字段(zero-core/claude/agents,display 用,`source` 仍管 app/user 逻辑)。
11. **SkillsPage 详情完善**— 用户反馈:详情页粗糙。source 标签改具体来源(ZERO-CORE/CLAUDE/AGENTS,替原始 app/user);body 用 MarkdownRenderer 渲染(view 模式);展示兄弟文件/脚本(新后端端点列目录)+ frontmatter 字段(触发词等)。

**依赖序**:sub-1(地基)→ sub-2(读家族核心,解析器)→ sub-4(prompt)→ **sub-9(接 runtime,skill 段运行时生效)**;sub-3(Shell)、sub-5(SkillsSection)、sub-6(SkillsPage CRUD)可并行/后置;sub-7←sub-6+sub-1;sub-8←sub-2+sub-4+sub-5;**sub-10←sub-5(introduces origin);sub-11←sub-6+sub-10(用 origin)+ 新后端端点**。

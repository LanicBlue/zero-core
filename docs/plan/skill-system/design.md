...
- 所以 skill 所有文件访问(md + 脚本)**统一走 `[skills]/` 虚拟路径**:Read 读正文/资源,Shell 跑 `${SKILL_DIR}/scripts/x.py`(替换后 = `[skills]/<id>/scripts/x.py`,Shell 解析前缀→真实路径→执行)。
...
4. **`[skills]/` 虚拟路径通道**:Read+Shell 识别前缀、解析真实路径、始终放行;`${SKILL_DIR}` 与 `${CLAUDE_SKILL_DIR}` **都**替换成 `[skills]/<id>`(兼容 Claude 生态 + 自有通用形式);真实路径 readScope 不变。
...
- **Glob/Grep 对 `[skills]/` 虚拟前缀的支持**:v1 defer(agent 靠 SKILL.md 里 `${SKILL_DIR}/x.md` 引用知道有哪些文件,不需列举);仅 Read+Shell 识别前缀。
...
→ `/effort plan`,6 sub 各配 acceptance(原 sub-2 拆为 Read/Shell 两段):
1. scanner 协议对齐(优先级 fix + id→dir 索引 + getSkillRoots + display name)
2. `[skills]/` Read 虚拟路径通道(解析器 + Read 前缀 + 始终放行 + 路径沙箱 + `${SKILL_DIR}`/`${CLAUDE_SKILL_DIR}` 替换;真实路径 readScope 不变)— **selection 段,核心**
...
6. `SkillsPage` 双栏 UI + 本软件 skill CRUD(目录形态;v1 管 SKILL.md 入口)

**依赖序**:sub-1(地基)→ sub-2(Read 核心)→ sub-4(prompt,至此 skill 链路可用);sub-3(Shell 脚本)、sub-5(SkillsSection)、sub-6(SkillsPage)可并行/后置(sub-5/6 仅依赖 sub-1)。

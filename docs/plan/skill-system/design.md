...
→ 第 1 段保留;**本 effort 补第 2/3 段(agent 直接读 skill 文件)+ per-agent 配置 + UI**。

## 方案:`[skills]/` 虚拟路径通道

协议的直读机制 + 我们的卫生化适配:agent 用**虚拟路径** `[skills]/<name>/<rel>` 访问任意 skill 文件,不接触真实磁盘路径。

### 解析与权限
- **Read / Shell** 识别 `[skills]/<name>/<rel>` 前缀 → 按 `<name>`(目录名=id)经 scanner 索引解析到真实 baseDir → join `<rel>` → 真实路径。
- `[skills]/` 通道**始终放行**(受信 skill 读取入口),**不经 readScope**。
- **真实路径访问不变**:`readScope="workspace"` 的 agent 用真实路径越界照样拒(`file-read.ts:57` 不动)。虚拟通道是唯一的 skill 受信入口。
- 路径沙箱:`<rel>` resolve 后必须仍在该 skill baseDir 内,拒 `../` 越界(防借 `[skills]/a/../../etc` 逃逸)。

### `${SKILL_DIR}` 替换(协议可移植自引用)
- 协议里 skill 作者用 `${SKILL_DIR}/reference.md` 可移植引用本 skill 文件。**我们读 skill 内容时把 `${SKILL_DIR}` 替换成 `[skills]/<name>`**,agent 看到的全是具体虚拟路径。
- 所以 skill 所有文件访问(md + 脚本)**统一走 `[skills]/` 虚拟路径**:Read 读正文/资源,Shell 跑 `${SKILL_DIR}/scripts/x.py`(替换后 = `[skills]/<name>/scripts/x.py`,Shell 解析前缀→真实路径→执行)。

### 否决的替代
- ❌ 专用 `skill` 工具(file/list 参数):协议**没有**专用 loader,agent 直接 Read;自造工具偏离协议 + 多余抽象。作废。
...
1. **identity = 目录名(id)**;display name = frontmatter `name` || 目录名。enabledSkills 持久化 **id(目录名)**(稳定,扛 frontmatter name 改动);prompt 展示用 display name;agent 寻址用 `[skills]/<id>/`。
...
4. **`[skills]/` 虚拟路径通道**:Read+Shell 识别前缀、解析真实路径、始终放行;`${SKILL_DIR}`→`[skills]/<id>` 替换;真实路径 readScope 不变。
5. **新 agent 默认全不开**(`enabledSkills = []`)。`system-prompt.ts:66-69` 的 `undefined` 分支不动(legacy agent 兼容,undefined=注入全部);显式 `[]`=空。
6. **作用域 = agent 级**(非 project):`AgentEditor` 加 `SkillsSection`(镜像 `ToolsSection.tsx`)。
7. **UI = 左列表 + 右详情**:`SkillsPage` 双栏,按来源分组(本软件置顶、外部其下)。
8. **可编辑边界**:只有"本软件 skills"(`~/.zero-core/skills/`)可新建/编辑/删除;外部来源**只读**。
...
- **plugin skill 命名空间 / marketplace**:不在范围。
- **`disable-model-invocation` frontmatter**:v1 不实现(可选增强)。

## 安全(协议指引,不造机制)

协议明确"只装可信来源、用前审计"。外部 skill(`~/.claude`)只读但可加载,其脚本可指示 agent 跑 bash → 不可信代码执行面(框架本就允许 bash,非新增能力)。继承协议指引,文档警示,不造额外机制。
...
→ `/effort plan`,5 sub 各配 acceptance:
1. scanner 协议对齐(优先级 fix + name→dir 索引 + getSkillRoots + display name)
2. `[skills]/` 虚拟路径通道(Read+Shell 前缀解析 + 始终放行 + `${SKILL_DIR}` 替换 + 路径沙箱;真实路径 readScope 不变)
3. prompt 注入(name+desc + `[skills]/<id>/` 寻址指引 + 加载/资源/脚本说明)+ 默认全不开
4. `SkillsSection`(agent 配置,enabledSkills 存 id)

...
- **指针格式 + 外置路径(sub-2 已定)**:外置文件 = `<ZERO_CORE_DIR>/tool-outputs/<sha256>.txt`(与 archives/wiki 同根,内容 hash 幂等不重复写,写盘失败回退原样存)。steps.content 里 tool 块的 `result` 字段换成自描述指针串:`[externalized: <REL_PATH> (<N> bytes)] <SUMMARY>`(REL_PATH 带 `.zero-core/` 前缀自描述根;N=原始 UTF-8 字节数;SUMMARY=头部 240 字单行化摘要)。导出 `parseExternalizedPointer`/`resolvePointerRelPath` 供 sub-3 fresh tail 渲染 / 按需寻回。tool 类型策略(昂贵/不可重跑保留)本轮**无差别 >16K 外置**,留 TODO。

...
- **拆除旧压缩引擎(本 sub 认领)**:删 `compression-engine.ts`(L1/L2、`identifyTurns`、`TurnBoundary`)+ 旧配置键(`l1Threshold 0.7` 等);`syncTurnsAfterCompression`/`replaceStepsFromMessages`(sub-1 过渡保留的破坏式路径)在此拆除。新引擎取代后无死代码残留。
- 不动 `steps` 表。

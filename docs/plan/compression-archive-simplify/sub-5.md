...
- **删 compaction/context 死模块**:[compaction.ts](../../../src/core/compaction.ts) + [context-manager.ts](../../../src/core/context-manager.ts) + `src/index.ts` 的 re-export。
- **删配置面**:`compaction.*` / `context.*`(只喂死模块)+ settings UI 引用。`compaction.*` 已被 sub-3 的可配压缩 prompt 取代。
- **删 steps.compressed 列**:[session-db.ts](../../../src/server/session-db.ts) schema + `db-migration.ts`(同步,否则 fresh DB 缺列——见项目记忆)。

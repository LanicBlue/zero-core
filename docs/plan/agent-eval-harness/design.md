# Design: agent-eval-harness 与配置化 Flow 文档面

> **状态**：设计已确认，已进入执行 plan；尚未实施。
> **日期**：2026-07-17。
> **问题记录**：[issue.md](./issue.md)。
> **研究输入**：[research.md](./research.md)。
...
│   │   └── <definition-id>/
│   │       └── <version>.yaml
│   ├── active.json                 # Project 当前默认 definition/version
│   ├── instances/
│   │   └── <flow-instance-id>/
│   │       ├── state.json
│   │       ├── dependencies.json
...
```text
Project
├── 一个 active FlowDefinition（常见情况）
├── 同一 definition 的历史版本
└── 可选的其他命名 FlowDefinition
```

项目注册或启用 Flow 时可以从内置/用户模板复制一份，但复制后由该 Project 自主管理，
不与模板保持实时链接。对于同一个 Project，FlowDefinition 一般稳定；只有状态、
transition 或 actor 语义改变时才增加版本。

- 新 FlowInstance 使用 Project 当前 active definition。
- 已存在 FlowInstance 固定引用创建时的 definition version + digest。
- 每个语义版本保存为独立、不可变的 `<definition-id>/<version>.yaml`；`active.json`
  只切换新实例默认使用的版本，运行时不需要临时 checkout 内层 Git 历史。
- 修改 active definition 不改变在途 FlowInstance 的解释。
...
本 effort 当前先等待 `wiki-system-redesign` 的顺序只是外部工作安排，并不存在于
zero-core 的 dependency graph 中。Plan 00 只在实施开始时核对该事实，不是 Flow 门禁，
...
| D7 | Flow 定义、实例、文档、事件和 Work 定义由内层 Git 版本化；runs/worktrees/cache/tmp 不跟踪。 |
...
| D10 | FlowDefinition 默认归 Project 所有，同一 Project 通常使用一个稳定 active definition。 |

...
acceptance，应先记录证据并回到本设计讨论，不得自行弱化 D1–D34、不变量或验收条件。
...
- Flow transition graph 允许项目配置回边/循环；正向和反向 transition 都追加标准 event，
  都能触发匹配 Work，只有 dependency/lineage graph 强制无环。
- 接棒 Agent 可以通过配置 transition 把 Ready→Discuss、Build→Plan、Verify→Build；
  返工 reason 按 transition input contract 审计，返工产生新 WorkRun 而不是旧 run retry。
- FlowInstance 可以经配置 transition 进入 terminal abandoned；废案保留历史并停止既有
  活动 WorkRun，不物理删除或级联废弃 dependent。
- WorkRun 成功不等于 Flow 自动迁移；Agent 通过通用 Flow tool 明确请求 transition。
- 工具权限保持按工具名：管理 Agent 的 Project 管 Flow/Work definition；普通 Project
  Agent 的 Flow 管 FlowInstance，Work 管 WorkRun。
- WorkRun 的 FIFO/priority 是默认建议；持有 Work 的 Agent可以 defer、prioritize、
  switch，但不能任意设置 terminal 状态或修改 definition snapshot。

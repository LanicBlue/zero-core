...
1. **context 块空也注入**:recall 未接入,work session 移走工作信息后 `memoryContext` 为空。**空也照常注入**(`buildContextMessage` 仍返 `## Recalled Memories` 段,内容空),保持通道常在 —— recall 接入后自然填,不用判"块在不在"。**不**因空而 skip。
2. **Wiki Anchors 合并落点 = 扩展现有 `wiki-system-anchors` system 段**,不新建段。`renderSystemAnchors` + `renderContextAnchors` 合一进该段(根 summary + 一层),context 块不再单独渲染 anchors 段。避免两段重复。

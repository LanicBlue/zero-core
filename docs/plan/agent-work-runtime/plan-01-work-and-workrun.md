# Plan 01：WorkDefinition、Trigger 与持久 WorkRun
...
Acceptance 00 通过；`project-flow-system` Final 提供的 Flow event/tool contract 不变。
...
普通 Agent 的 `Work` 工具。本 effort Plan 05 由 management-only `Project.config.*`
暴露完整版本 publish/activate。
...
`switch` 在本阶段只用 fake session handoff 验证双 revision CAS，Plan 02 再接真实
...
本阶段用 fake dispatcher 测状态机。真实 Agent dispatch 在 Plan 02 接入。旧
...
[Acceptance 01](acceptance-01-work-and-workrun.md) 全部通过并生成 `result-01.md`。

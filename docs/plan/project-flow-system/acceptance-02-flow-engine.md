...
- [ ] 同 Project 可同时维护多个 active binding；切换一个 definitionId 不改变其他 binding。
...
- [ ] related 与 dependency/composition 使用独立 schema 和索引。
- [ ] related 可同/跨 Project导航，但不参与 cycle、milestone、gate 或 terminal-blocked。
- [ ] 未授权跨 Project related 不泄露目标内容。

## E. Tool 与隔离

- [ ] 未注册 runtime action factory 只有通用 action，无 fixed ready/build/verify 分支。
- [ ] runtime contract 不包含 FlowDefinition publish/activate/mutation。
- [ ] contract 从 definition 列出并执行返工 transition，不硬编码 reject/rollback action。
- [ ] contract 暴露 dependency、related、split、merge 和 lineage 通用 action。
- [ ] 本阶段不伪装成正式 Agent-facing Flow 工具已经切换。
...
- [ ] eventId 重放幂等，fake consumer 不产生第二次 side effect；本阶段没有真实
  WorkRunStore。
...
milestone 在返工后的差异，以及 terminal event ordering/依赖矩阵。

...
同一 Project 可以启用多个命名 FlowDefinition。active 配置是
`definitionId → version + digest` binding map，并可选一个 `defaultDefinitionId`；切换一个
binding 不得改变其他 Definition。FlowInstance 显式指定 definitionId，省略时才使用
default，并固定创建时的 version/digest。
...
- set/get active binding map 与 defaultDefinitionId；
- create/list/get FlowInstance；
- list allowed transitions；
- transition with `expectedRevision`；
- list event/history。
- add/remove/list `related` relation；related 与 dependency/composition 分库，不参与
  milestone、cycle 或 gate。
...
event 新 Work”的顺序合同；实际 WorkRun cancel/request-cancel 在
Agent Work Runtime Plan 01 接入。
...
### 7. 通用 Flow runtime contract

提供可供测试直接调用、但尚未替换生产注册的 runtime action factory：
`list/get/create/transition/history/dependencies.add/remove/list/status/
related.add/remove/list/split/merge/lineage`。
contract 只管理 FlowInstance runtime，不包含 FlowDefinition publish/activate/mutation；
definition validator/repository 在本阶段提供 service。本 effort Plan 03 再原子切换正式
Flow 工具并通过 management-only `Project.config.*` 暴露配置。
身份和 active Project 从 CallerCtx 注入；显式跨 Project操作仅允许已注册目标和 Agent
policy 授权，不信任 LLM 伪造 actor。

旧 Requirement service/UI/Flow tool 暂时保留且不与新模型双写；Plan 03 切换前不能把
未注册 factory 描述成 Agent 已可用。

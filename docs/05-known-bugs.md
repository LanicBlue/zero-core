...
> 最近重写：2026-06（B3/B4/B8 已修，重排剩余项）
...
**问题**：
- 如果 user 还没发第一条消息，没有 assistant 气泡可附加 — error 文本无处显示
...
**未覆盖**：多轮对话、工具调用链路、Thinking / reasoning 流式、Recovery（kill + restart）、MCP tool 调用、KB 检索、agent 编辑后续聊、session 删除。

...
// ─── PM role preset allowlist ─────────────────────────────────

describe("PM role preset (defect 1 — tool allowlist)", () => {
	test("PM preset enables CreateRequirementWithDoc and whitelists analyzer", () => {
		const preset = getPreset("pm")!;
		expect(preset.toolPolicy.tools?.CreateRequirementWithDoc).toEqual({ enabled: true });
		// PM stays read-only to the filesystem (no Write/Edit).
		expect(preset.toolPolicy.tools?.Write).toBeUndefined();
		expect(preset.toolPolicy.tools?.Edit).toBeUndefined();
		// Analyzer agent-tool edge still declared (PM may call analyzer).

	...
	return [
		{
			...
			name: "需求管理",
			actionPrompt: [
				`有新需求进入项目「${projectName}」。你的职责是接手并推进交付:`,
				"",
				"1. 用 Read/Grep/Glob + Wiki 读项目代码现状与既有 wiki,理解上下文。",
				"2. 用 Orchestrate 工具编排交付流程:把需求拆成 task 节点,每个节点指定一个 subagent 执行。",
				"3. 提交编排(plan 门),确认后流程自动推进。",
				"4. 流程跑完 + 自验通过后,显式调用 verify 工具提交覆盖判断。",
				"",

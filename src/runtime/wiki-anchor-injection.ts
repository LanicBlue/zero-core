...
function classifyAnchorKind(nodeId: string, node: WikiNode | undefined): "project" | "memory" {
	...
	// Recognized by id prefix so a per-agent root resolves to memory-kind EVEN
	// BEFORE its row is lazily created (the memory anchor is synthetic; the row
	// is ensured by management-service / the createMemory lazy-ensure).

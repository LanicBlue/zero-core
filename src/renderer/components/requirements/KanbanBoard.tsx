// 看板子组件 (v0.8 P5 §8.5 — 项目页的「看板」tab)
//
// # 文件说明书
//
// ## 核心功能
// 按状态分列展示需求卡片 + plan-gate review + 创建/讨论/coverage 入口。
// 这是从原 KanbanPage 抽出的纯看板部分;项目页接管了项目列表 / 选择 / 新建,
// 本组件只负责一个项目(或全部)的需求看板视图。
//
// ## 输入
// - props.projectId  当前要看的项目 id("" = 全部,跨项目看板)
// - useRequirementStore / useProjectStore (Zustand)
//
// ## 输出
// - 渲染的看板列(被 ProjectPage 的「看板」tab 内嵌)
//
// ## 定位
// 渲染进程子组件,被 ProjectPage 嵌入。原 KanbanPage 的项目选择/toolbar 已
// 迁出,本组件保持纯看板逻辑(列划分、卡片交互、plan-gate、coverage 等)。
//
// ## 依赖
// - react
// - ../../store/* (requirement / project / page)
// - ./RequirementCard.js / ./CreateRequirementModal.js /
//   ./ExecutionDetailPanel.js / ./CoverageJudgementModal.js
//
// ## 维护规则
// - 看板列划分或状态机变更同步 KANBAN_COLUMNS
// - 新需求操作挂接进回弹窗或卡片回调
// - 项目选择由父级 ProjectPage 控制,本组件不再渲染项目 select
//
...
/**
 * @param projectId  current project filter ("" = all projects).
 */
export default function KanbanBoard({ projectId }: { projectId: string }) {
	...
	const fetchPendingPlans = useCallback(async (pid?: string) => {
		try {
			const api = (window as any).api;
			if (!api?.orchestratePending) return;
			const plans = await api.orchestratePending(pid ? { projectId: pid } : undefined) as OrchestratePlanRecord[];
	...
	const handleConfirmPlan = useCallback(async (planId: string) => {
		...
		try {
			...
			await fetchPendingPlans(projectId || undefined);
		} catch (e) {
	...
	return (
		<div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
			{/* Toolbar (slim — project selection lives in the parent ProjectPage) */}
			<div style={{
				display: "flex", alignItems: "center", gap: 12,
				padding: "8px 12px", borderBottom: "1px solid var(--border-color, #333)",
			}}>
				<span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary, #e0e0e0)" }}>
					{"\u{1F4CB}"} Kanban {projectId && `· ${projects.find((p) => p.id === projectId)?.name ?? projectId}`}
				</span>
				<div style={{ flex: 1 }} />
				<button type="button" onClick={() => setShowCreateModal(true)} style={primaryBtnStyle}>
					+ New Requirement
				</button>
				<button type="button" onClick={handleRefresh} disabled={loading} style={ghostBtnStyle}>
					{loading ? "..." : "\u{1F504}"}
				</button>
			</div>
			...
			<div style={{ flex: 1, display: "flex", overflowX: "auto", gap: 0 }}>
				...
				{pendingPlans.length > 0 && (
					<div style={{
						...
						<div style={{
							padding: "10px 12px", borderBottom: "2px solid #FF9800",
							display: "flex", alignItems: "center", gap: 6,
							fontSize: 12, fontWeight: 600, color: "#FF9800",
						}}>
							<span>{"\u{23F3}"}</span>
									...
									<div key={plan.id} style={{
										...
										<div style={{ display: "flex", gap: 6 }}>
											<button
												...
												style={{ flex: 1, padding: "4px 8px", background: "#4CAF50", border: "none", borderRadius: 4, color: "#fff", fontSize: 11, cursor: "pointer" }}
											>
											...
											<button
												...
												style={{ flex: 1, padding: "4px 8px", background: "transparent", border: "1px solid #f44336", borderRadius: 4, color: "#f44336", fontSize: 11, cursor: "pointer" }}
											>
												Reject
											</button>
										</div>
											...
											<div style={{ marginTop: 6 }}>
												...
												<div style={{ display: "flex", gap: 6, marginTop: 4 }}>
													<button
														...
														disabled={isConfirming}
														style={{ flex: 1, padding: "3px 8px", background: "#f44336", border: "none", borderRadius: 4, color: "#fff", fontSize: 11, cursor: "pointer" }}
													>
														Send
													</button>
													<button
														...
														style={{ flex: 1, padding: "3px 8px", background: "transparent", border: "1px solid var(--border-color, #333)", borderRadius: 4, color: "var(--text-secondary, #888)", fontSize: 11, cursor: "pointer" }}
													>
														Cancel
													</button>
												</div>
